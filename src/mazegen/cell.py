...
class Cell:
    ...
    including walls (paths), special flags (start/exit), and
    algorithm-specific data (visited status, vectors).
    ...
    Attributes:
        ...
        visited (bool): Flag used by generation algorithms to track
            processing state or visual "chaos" propagation.
        forbidden (bool): True if the cell is part of a masked area
            (e.g., text) and cannot be part of the maze path.
        is_solution (bool): True if the cell is part of the solved path
            from Start to Exit. Used for rendering.
        vector (Optional[str]): The direction of the "flow" for algorithms
            like Origin Shift. Points to the parent cell or flow direction.
    """
    ...
    def get_info(self) -> str:
        ...
        Useful for debugging purposes to inspect the internal state.

        Returns:
            str: A string representation of the instance dictionary.
        """
    ...
    def set_path(self, direction: str, is_path: bool) -> None:
        ...
        Args:
            direction (str): The cardinal direction ("north", "south",
                "east", "west").
